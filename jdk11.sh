...
wget -O /tmp/jdk11.tgz https://corretto.aws/downloads/latest/amazon-corretto-11-x64-linux-jdk.tar.gz
sudo mkdir -p /usr/lib/jvm
sudo tar xf /tmp/jdk11.tgz -C /usr/lib/jvm
rm /tmp/jdk11.tgz
sudo ln -sf /usr/lib/jvm/amazon-corretto-11.*-linux-x64 /usr/lib/jvm/jdk11
VERSION=jdk11
PRIORITY=100
sudo chown -R root:root /usr/lib/jvm/${VERSION}
sudo update-alternatives --install /usr/bin/jaotc jaotc /usr/lib/jvm/${VERSION}/bin/jaotc ${PRIORITY}
...
sudo update-alternatives --install /usr/bin/pack200 pack200 /usr/lib/jvm/${VERSION}/bin/pack200 ${PRIORITY}
sudo update-alternatives --install /usr/bin/rmic rmic /usr/lib/jvm/${VERSION}/bin/rmic ${PRIORITY}
sudo update-alternatives --install /usr/bin/rmid rmid /usr/lib/jvm/${VERSION}/bin/rmid ${PRIORITY}
...
sudo update-alternatives --install /usr/bin/unpack200 unpack200 /usr/lib/jvm/${VERSION}/bin/unpack200 ${PRIORITY}
